...
calculate_hex_coords = function(shots, binwidths) {
  ...
  hexbin_stats = shots %>%
    ...
    summarize(
      ...
      hex_points_scored = sum(shot_made_flag * readr::parse_number(shot_type)),
      hex_points_per_shot = mean(shot_made_flag * readr::parse_number(shot_type))
    )
...
calculate_hexbins_from_shots = function(
  shots, binwidths = c(1, 1), min_radius_factor = 0.6,
  fg_diff_limits = c(-0.12, 0.12), fg_pct_limits = c(0.2, 0.7),
  ...
  zone_stats = grouped_shots %>%
    summarize(
      ...
      zone_points_scored = sum(shot_made_flag * readr::parse_number(shot_type)),
      zone_points_per_shot = mean(shot_made_flag * readr::parse_number(shot_type))
    )
...
generate_hex_chart = function(hex_data, metric = sym(bounded_fg_diff), alpha_range = c(0.85, 0.98)) {
  ...
  ggplot() +
    geom_polygon(
      data = hex_data$hex_data, #%>% 
        #mutate(hex_attempts = hex_attempts/max(hex_attempts)),
      aes_string(
        x = "adj_x",
        y = "adj_y",
        group = "hexbin_id",
       ...
       size = court_themes$dark$hex_border_size,
       color = court_themes$dark$hex_border_color
    ) +
...
train %>% 
  filter( shot_zone_basic != "Backcourt", season == "1996-97") %>% 
  mutate(loc_x = loc_x/10,
         loc_y = (loc_y + 50)/10) %>% 
  calculate_hexbins_from_shots(binwidths  = c(1, 1), fg_pct_limits =c(0.3, 0.55)) %>% 
  generate_hex_chart(metric = "bounded_fg_pct", alpha_range = c(0.4, 1)) +
  geom_path(
    data = court_points,
